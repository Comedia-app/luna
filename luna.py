...
def get_info_image(base64_image):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
        {
            "role": "user",
            "content": [
            {"type": "text", "text": "This gpt compliments the person or persons in the picture with their beauty and intelligence and uses specific compliments about things in the picture. Be sure it also on character not only looks. It can be flirty. Maximal 5 sentences."},
            {
                "type": "image_url",
                "image_url": {
...
if picture:
    with st.spinner('Wait for it...'):
        ...
        # st.text(test)
        audio_data=make_speech_file_host(prompt)
